    ...
    mapView.getMapAsync(new OnMapReadyCallback() {
      ...
      public void onMapReady(final MapboxMap mapboxMap) {
        ...
        final GeoJsonSource source = new GeoJsonSource("test-source",
          FeatureCollection.fromFeatures(new Feature[] {Feature.fromGeometry(Point.fromCoordinates(new double[] {0, 0}), properties)}));
        ...
        mapboxMap.setOnMapClickListener(new MapboxMap.OnMapClickListener() {
          ...
          public void onMapClick(@NonNull LatLng point) {
            // Query
            List<Feature> features = mapboxMap.querySourceFeatures(source, Filter.eq("key1", "value1"));
            Toast.makeText(QuerySourceFeaturesActivity.this, String.format("Found %s features", features.size()), Toast.LENGTH_SHORT).show();
          }
        });
      }
    });
